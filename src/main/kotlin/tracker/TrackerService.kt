...
class TrackerService(private val metainfo: Metainfo) {
    ...
    private val peerId = getRandomPeerId()
    ...
    fun getInfoFromTracker(): TrackerResponse {
        val queryMap = mutableMapOf<String, String>()
        queryMap[INFO_HASH]=metainfo.infoHashForUrl.urlEncode()
        queryMap[PEER_ID]=peerId.urlEncode()
        queryMap[PORT]=port.toString()
        queryMap[DOWNLOADED]="0"
        queryMap[UPLOADED]="0"
        queryMap[LEFT]=metainfo.torrentInfo.length?.toString() ?: metainfo.torrentInfo.files!!.sumOf { it.length }.toString()
